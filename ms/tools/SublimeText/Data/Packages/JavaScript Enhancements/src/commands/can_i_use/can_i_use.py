...
import re, json, os, traceback, webbrowser, cgi
...
path_to_can_i_use_data = os.path.join(os.path.dirname(os.path.abspath(__file__)), "can_i_use_data.json")
path_to_test_can_i_use_data = os.path.join(os.path.dirname(os.path.abspath(__file__)), "can_i_use_data2.json")
url_can_i_use_json_data = "https://raw.githubusercontent.com/Fyrd/caniuse/master/data.json"

can_i_use_css = ""
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "can_i_use.css"), encoding="utf-8") as css_file:
...
def back_to_can_i_use_list(action):
  ...
  if len(action.split(",")) > 1 and action.split(",")[1] == "main-menu" :
    view.run_command("javascript_enhancement_can_i_use", args={"from": "main-menu"})
  else :  
    view.run_command("javascript_enhancement_can_i_use")
...
def show_pop_can_i_use(index):
  ...
  popup_manager.set_visible("javascript_enhancements_can_i_use", True)
  view.show_popup("""
    <html>
      <head></head>
      <body>
    ...
    </html>""", sublime.COOPERATE_WITH_AUTO_COMPLETE, -1, 1250, 650, back_to_can_i_use_list, lambda: popup_manager.set_visible("javascript_enhancements_can_i_use", False))

class javascript_enhancements_can_i_useCommand(sublime_plugin.TextCommand):
