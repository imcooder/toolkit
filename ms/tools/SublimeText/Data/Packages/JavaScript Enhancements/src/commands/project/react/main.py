...
def react_prepare_project(project_path, react_custom_path):
  ...
  if sublime.platform() != "windows": 
    ...
    terminal.run([react_custom_path, "my-app", "&", os.path.join(WINDOWS_BATCH_FOLDER_PATH, "move_all.bat"), "my-app", ".", "&", "rd", "/s", "/q", "my-app"])
    if open_project:
...
# class JavascriptEnhancementsEnableReactMenuEventListener(JavascriptEnhancementsEnableProjectTypeMenuEventListener, sublime_plugin.EventListener):
#   project_type = "react"
...
# class JavascriptEnhancementsReactCliCommand(JavascriptEnhancementsExecuteOnTerminalCommand, sublime_plugin.WindowCommand):
