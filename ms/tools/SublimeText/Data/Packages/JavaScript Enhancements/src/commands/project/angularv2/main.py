...
class JavascriptEnhancementsAngularv2CliCommand(JavascriptEnhancementsExecuteOnTerminalCommand, sublime_plugin.WindowCommand):
  ...
  def _run(self):
    ...
    super(JavascriptEnhancementsAngularv2CliCommand, self)._run()
...
def angularv2_prepare_project(project_path, angularv2_custom_path):
  ...
  if sublime.platform() != "windows": 
    ...
    terminal.run([angularv2_custom_path, "new", "my-app", "HelloWorld", "&", os.path.join(WINDOWS_BATCH_FOLDER_PATH, "move_all.bat"), "my-app", ".", "&", "rd", "/s", "/q", "my-app"])
    if open_project:
