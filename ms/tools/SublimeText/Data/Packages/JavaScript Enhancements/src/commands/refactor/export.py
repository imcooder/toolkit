...
class JavascriptEnhancementsRefactorExportCommand(sublime_plugin.TextCommand):
  def run(self, edit, **args):
    ...
    if view.is_dirty():
      sublime.error_message("Cannot export " + tp + ". There are unsaved modifications to the buffer. Save the file before use this.")
      return 
    ...
    if settings:
      ...
      if result[0]:
        ...
        items_sorted = sorted(items, key=lambda item: int(item['loc']['start']['line']))
        import_regions = []
        need_to_import = True
        ...
        if need_to_import:
          ...
          if not args.get("preview"):
            ...
            splitted_content = splitted_content[:line + 1] + text.lstrip().splitlines() + ([""] if not is_next_line_empty else [])  + splitted_content[line + 1:]
            range_start = max(0, line - 2)
            range_end = min(line + 2, len(splitted_content) - 1)
            preview_content = "- Import to\n" + file_name + ":\n\n"
            while range_start <= range_end:
